...
def sggsvdOpts(A, B):
    ...
    beta = np.zeros(
        n
    )  #(output) alpha and beta contain generalized singular values of the matrix A and B
    if jobu == "U":
        ldu = max(m, 1)
    ...
    info = 0
    return (jobu, jobv, jobq, m, n, p, k, l, lda, ldb, alpha, beta, ldu, U,
            ldv, V, ldq, Q, work, iwork, info)


def gsvd(A, B):
    #Generate command line options for calling LAPACK Fortran Routine for two matrices
    (jobu, jobv, jobq, m, n, p, k, l, lda, ldb, alpha, beta, ldu, U, ldv, V,
     ldq, Q, work, iwork, info) = sggsvdOpts(A, B)
    #Call the Fortran wrapped generalized SVD routine for real-numbered matrices
    k, l, a, b, alpha, beta, u, v, q, work, iwork, info = sggsvd.sggsvd(
        jobu=jobu,
        jobv=jobv,
        jobq=jobq,
        m=m,
